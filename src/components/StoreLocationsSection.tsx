      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {STORES_INFO.map((store) => {
          ...
          return (
            <div
              key={store.id}
              className="glass-panel rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-700 transition-all group bg-white dark:bg-slate-900 shadow-sm"
            >
                ...
                <div className="absolute top-4 left-4">
                  {isLighthouse ? (
                    <span className="glass-badge-gold px-3 py-1 rounded-full text-xs font-extrabold flex items-center gap-1.5 shadow-sm">
                      <Lightbulb className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" /> Light House Showroom
                    </span>
                  ) : (
                    <span className="glass-badge-blue px-3 py-1 rounded-full text-xs font-extrabold flex items-center gap-1.5 shadow-sm">
                      <Zap className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" /> Electrical Shop
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-400 font-semibold">{store.tagline}</p>
                  ...
                  {/* Specialties List */}
                  <div className="pt-2">
                    <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-2">Showroom Specialties</span>
                    <div className="flex flex-wrap gap-1.5">
                      {store.specialties.map((spec, i) => (
                        <span key={i} className="text-[11px] px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 flex items-center gap-1 font-medium">
