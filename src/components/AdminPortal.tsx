...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/85 backdrop-blur-lg overflow-y-auto">
      <div 
        className="glass-panel w-full max-w-4xl rounded-3xl overflow-hidden border border-slate-200 dark:border-white/20 bg-white dark:bg-slate-900 shadow-2xl relative my-6 text-left"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Top Header Bar */}
        <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-500">
              <ShieldCheck className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Owner & Admin Management Portal</h2>
              <p className="text-xs text-slate-500 dark:text-gray-400">Upload and adjust lights & appliance prices and catalog</p>
            </div>
          </div>
        ...
        {!isAdminLoggedIn ? (
          <div className="p-8 sm:p-12 text-center max-w-md mx-auto space-y-6">
            <div className="w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-500 flex items-center justify-center mx-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-white/10 pb-4">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-xs sm:text-sm flex items-center gap-2 transition-all ${
                    activeTab === 'add' && !editingProduct
                      ...
                      : 'glass-button-secondary text-slate-700 dark:text-gray-300'
                  }`}
                >
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={onResetData}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white glass-button-secondary flex items-center gap-1"
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700"
                >
                  Logout
                </button>
              </div>
            ...
            {activeTab === 'add' && (
              <form onSubmit={handleSubmit} className="space-y-5 max-h-[65vh] overflow-y-auto pr-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Select Target Shop</label>
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 border transition-all ${
                          shopOrigin === 'lighthouse'
                            ...
                            : 'glass-button-secondary text-slate-700 dark:text-gray-400'
                        }`}
                      >
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Product Category</label>
                    <select
                      value={category}
                      onChange={(e) => setCategory(e.target.value)}
                      className="w-full p-2.5 rounded-xl text-xs glass-input focus:outline-none"
                    >
                      {(shopOrigin === 'lighthouse' ? LIGHT_CATEGORIES : APPLIANCE_CATEGORIES)
                        .filter((c) => !c.startsWith('All'))
                        .map((cat, idx) => (
                          <option key={idx} value={cat} className="bg-white dark:bg-gray-900 text-slate-900 dark:text-white">
                            {cat}
                          </option>
                        ))}
                    </select>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Product Title *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Brand Name</label>
                    <input
                      type="text"
                ...
                {/* Price Section */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Original Price (₹) *</label>
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-xs text-slate-500 dark:text-gray-400">₹</span>
                      <input
                        type="number"
                        placeholder="45000"
                        value={originalPrice}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-amber-600 dark:text-amber-500 mb-1">Discount Price (₹) [Optional]</label>
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-xs text-amber-600 dark:text-amber-500">₹</span>
                      <input
                        type="number"
                        placeholder="32999"
                        value={discountPrice}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-emerald-600 dark:text-emerald-400 mb-1">Savings Preview</label>
                    <div className="p-2 text-xs font-bold text-emerald-600 dark:text-emerald-300 pt-3">
                      {originalPrice && discountPrice && parseFloat(originalPrice) > parseFloat(discountPrice) ? (
                        ...
                        <span className="text-slate-500 dark:text-gray-500 font-normal">No discount active</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {/* Image URL & Presets */}
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Product Image URL</label>
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto pt-1">
                    <span className="text-[11px] text-slate-500 dark:text-gray-400 whitespace-nowrap">Quick Image Presets:</span>
                    {sampleImages.map((s, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => setImageUrl(s.url)}
                        className="text-[10px] px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-gray-300 border border-slate-200 dark:border-white/10 whitespace-nowrap font-medium"
                      >
                        + {s.label}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 mb-1">Product Description</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs font-bold text-slate-700 dark:text-gray-300">Quick Specifications</label>
                    <input
                      type="text"
                ...
                <div className="pt-3">
                  <button
                    type="submit"
                    className="w-full py-3.5 rounded-xl glass-button-primary text-white font-extrabold text-sm shadow-xl flex items-center justify-center gap-2"
                  >
              ...
              <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <span className="text-xs text-slate-600 dark:text-gray-400">Filter Shop:</span>
                    <button
                      onClick={() => setFilterShop('all')}
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${
                        filterShop === 'all' ? 'bg-slate-200 text-slate-900 dark:bg-white/20 dark:text-white' : 'text-slate-600 dark:text-gray-400'
                      }`}
                    >
                      All
                    </button>
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${
                        filterShop === 'lighthouse' ? 'bg-amber-600 text-white' : 'text-slate-600 dark:text-gray-400'
                      }`}
                    >
                      Light House
                    </button>
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${
                        filterShop === 'electrical_shop' ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-gray-400'
                      }`}
                    >
                      Electrical Shop
                    </button>
                ...
                {/* Table of Products */}
                <div className="rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden bg-slate-50 dark:bg-black/40 text-xs">
                  <div className="divide-y divide-slate-200 dark:divide-white/10">
                    {filteredProducts.length === 0 ? (
                      <div className="p-8 text-center text-slate-500 dark:text-gray-400">No products match your filter criteria.</div>
                    ) : (
                      filteredProducts.map((p) => {
                        const hasDisc = p.discountPrice && p.discountPrice < p.originalPrice;
                        return (
                          <div key={p.id} className="p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-2 mb-0.5">
                                  ...
                                  <span className="text-slate-500 dark:text-gray-400">{p.category}</span>
                                </div>
                                <h4 className="font-bold text-slate-900 dark:text-white text-sm">{p.title}</h4>
                                <span className="text-[11px] text-slate-500 dark:text-gray-400">{p.brand}</span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                              <div className="text-left sm:text-right">
                                ...
                                {hasDisc && (
                                  <div className="text-[10px] text-slate-400 dark:text-gray-400 line-through">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  className="p-2 rounded-lg bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700"
