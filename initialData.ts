      ...
      <main className="flex-1">
        ...
        {activeTab === 'stores' ? (
          ...
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-5">
            ...
            {/* Filter & Controls Bar - Height and Width Adjusted */}
            <div className="max-w-6xl mx-auto glass-panel p-2.5 sm:p-3 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-2 shadow-sm">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition-all whitespace-nowrap h-7.5 flex items-center ${
                      selectedShop === 'all'
                        ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 shadow-sm font-extrabold'
                        : 'glass-button-secondary text-slate-600 dark:text-slate-300'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap h-7.5 ${
                      selectedShop === 'lighthouse'
                        ? 'bg-amber-600 text-white shadow-sm font-extrabold'
                        : 'glass-button-secondary text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30'
                    }`}
                  >
                    <Lightbulb className="w-3.5 h-3.5" /> Light House
                  </button>
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap h-7.5 ${
                      selectedShop === 'electrical_shop'
                        ...
                        : 'glass-button-secondary text-blue-700 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30'
                    }`}
                  >
                    <Zap className="w-3.5 h-3.5" /> Electrical Shop
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400 cursor-pointer glass-card px-2.5 h-7.5 rounded-lg hover:border-emerald-500 transition-all">
                    <input
                      type="checkbox"
                    ...
                    <span className="whitespace-nowrap text-[11px] sm:text-xs">Discount Offers Only</span>
                  </label>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                    ...
                    <select
                      ...
                      className="glass-input h-7.5 px-2.5 rounded-lg text-xs focus:outline-none focus:border-amber-500 cursor-pointer"
                    >
              ...
              {/* Category Pills Bar */}
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex items-center gap-1.5 overflow-x-auto no-scrollbar text-xs">
                <span className="text-slate-400 font-semibold text-[10px] shrink-0 mr-1 uppercase tracking-wider">Categories:</span>
                
                <button
                  onClick={() => setSelectedCategory('all')}
                  className={`px-2.5 py-1 rounded-md text-xs transition-all shrink-0 h-7 flex items-center ${
                ...
                {activeCategoryList
                  ...
                  .map((cat, idx) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md text-xs transition-all shrink-0 whitespace-nowrap h-7 flex items-center ${
            ...
            {sortedProducts.length === 0 ? (
              <div className="glass-panel p-12 rounded-3xl text-center space-y-4 my-8">
                <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 text-gray-500 flex items-center justify-center mx-auto">
